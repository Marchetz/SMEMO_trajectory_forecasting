...
class NTMMemory(nn.Module):
    ...
    def __init__(self, N, M):
        ...
        #todo: provare memoria constante init
        #pdb.set_trace()
    ...
    #     if temp.all():
    #         w = torch.zeros(sim_2.shape).cuda()
    #     return w
    ...
    def similarity_mask_inf(self, k, β, memory_total, mask):
        ...
        w = F.softmax(sim_2, dim=1)
        if len(index_all_zeros) > 0:
            mask_softmax = torch.ones(w.shape).to(torch.device('cuda'))
            mask_softmax[index_all_zeros] = 0.0
            w = w * mask_softmax

        return w

    def similarity_mask_inf_v2(self, k, β, memory_total, mask):
        k = k.unsqueeze(1)
        sim = β * (F.cosine_similarity(memory_total + 1e-16, k + 1e-16, dim=-1))
        sim[mask == 0] = -float("inf")

        index_all_zeros = torch.where((mask == 0.0).all(dim=1))[0]
        if len(index_all_zeros) > 0:
            sim[index_all_zeros] = 0.0
